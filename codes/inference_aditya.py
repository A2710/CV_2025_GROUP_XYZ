...
def get_system_metrics():
    ...
    if gpu_handle:
        try:
            ...
            metrics["gpu_name"] = pynvml.nvmlDeviceGetName(gpu_handle).decode()
...
def load_icfg_pedes_data(dataset_dir, split="test", batch_size=4):
    """Load and preprocess ICFG-PEDES dataset using nvImageCodec and CVCUDA."""
    ...
    for i in tqdm(range(0, len(df), batch_size), desc=f"Loading {split} data"):
        ...
        for _, row in batch_df.iterrows():
            ...
            if not isinstance(img_path, str):
                logger.error(f"Invalid img_path: {img_path}")
                raise ValueError(f"Invalid img_path: {img_path}")
            try:
                nv_img = decoder.read(img_path, cuda_stream=cvcuda_stream.handle)
                batch_images.append(torch.from_numpy(nv_img).to("cuda"))
            except Exception as e:
                logger.error(f"Failed to decode {img_path}: {e}")
                continue
        ...
        nv_tensor = cvcuda.as_tensor(torch.stack(batch_images), "NHWC")
        ...
        texts.extend(batch_df['description'].tolist())
        labels.extend(batch_df['id'].tolist())

        # Clear GPU memory
        del batch_images, nv_tensor, nv_resized, nv_float, nv_normalized
