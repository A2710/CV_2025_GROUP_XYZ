...
def prepare_calibration_data(dataset_path: str, batch_size: int, input_shape: Tuple, num_batches: int = 10) -> np.ndarray:
    ...
    Args:
        dataset_path: Path to the ICFG-PDES dataset
...
def build_engine(
    ...
    with trt.Builder(TRT_LOGGER) as builder, \
        ...
        # Support batch sizes from 1 to max batch_size
...
def run_inference(engine: trt.ICudaEngine, input_data: np.ndarray) -> List[np.ndarray]:
    ...
    with engine.create_execution_context() as context:
        ...
        for name in binding_names:
            ...
            # Validate output shape against expected [batch_size, 1000]
            expected_shape = (batch_size, 1000)
            if tuple(output_shape) != expected_shape:
                raise ValueError(
                    f"Unexpected output shape for '{name}': {output_shape}. "
                    f"Expected: {expected_shape}"
                )

            # Calculate output_size as product of dimensions
            output_size = 1
            for dim in output_shape:
                if dim <= 0:
                    raise ValueError(f"Invalid dimension in output shape: {dim}. All dimensions must be positive.")
                output_size *= dim

            output_dtype = np.dtype(trt.nptype(engine.get_tensor_dtype(name)))  # Convert to NumPy dtype instance
            total_size = output_size * output_dtype.itemsize
            print(f"Output '{name}': output_shape={output_shape}, output_size={output_size}, "
                  f"output_dtype={output_dtype}, total_size={total_size}")

            # Validate total_size
            if total_size <= 0:
                raise ValueError(f"Invalid memory allocation size: {total_size}. Must be positive.")
            if total_size > 2**31 - 1:
                raise ValueError(f"Memory allocation size {total_size} exceeds maximum allowed value.")

            # Allocate CUDA memory
            d_output = cuda.mem_alloc(total_size)
...
def main():
    ...
    onnx_model_path = "/media/seas_au/prsdata/xyz/cv_final/ICFG-PDES/ICFG-PEDES/final_model.onnx"
